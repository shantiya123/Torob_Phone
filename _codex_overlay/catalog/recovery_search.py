...
class SearchRecoveryService:
    ...
    def generate_plans(self, *, original_query_set, max_candidates=MAX_TESTED_CANDIDATES, max_plans=MAX_RETURNED_PLANS):
        ...
        logger.info(
            "recovery_started",
            extra={"active_fields": [_field_name(path) for path, _policy, _value in active], "candidate_limit": candidate_limit},
        )
                ...
                try:
                    ...
                    logger.warning(
                        ...
                        extra={
                            ...
                            "operation": policy.operation if proposed_value is not None else "make_optional",
                            "candidate_index": candidates_tested,
        ...
        logger.info(
            ...
            extra={"candidates_tested": candidates_tested, "plans_found": len(numbered)},
        )
        if numbered:
            logger.info("recovery_plans_found", extra={"plans_found": len(numbered)})
        else:
            logger.info("recovery_no_safe_plan", extra={"candidates_tested": candidates_tested})
