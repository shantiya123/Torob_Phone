...
class UnsupportedQuerySetFieldError(ValueError):
    def __init__(self, fields):
        ...
        super().__init__("Unsupported QuerySet filters: " + ", ".join(self.fields))


_BRAND_ALIASES = {
    "apple": "Apple", "Ø¢Ù¾Ù„": "Apple", "Ø§Ù¾Ù„": "Apple", "Ø¢ÛŒÙÙˆÙ†": "Apple", "Ø§ÙŠÙÙˆÙ†": "Apple",
    "samsung": "Samsung", "Ø³Ø§Ù…Ø³ÙˆÙ†Ú¯": "Samsung",
    "xiaomi": "Xiaomi", "Ø´ÛŒØ§Ø¦ÙˆÙ…ÛŒ": "Xiaomi", "Ø´Ø§ÙˆÙ…ÛŒ": "Xiaomi",
    "poco": "Poco", "Ù¾ÙˆÚ©Ùˆ": "Poco",
}
...
def _ensure_unsupported_fields_are_empty(query_set):
    ...
    if unsupported:
        raise UnsupportedQuerySetFieldError(unsupported)
...
def query_set_to_filter_requirements(query_set):
    ...
    return FilterRequirements(
        ...
        required_ip_rating=physical["ip_rating"], price_min=query_set["price"]["min"], price_max=query_set["price"]["max"],
        minimum_android_version=software["android_version"]["min"],
